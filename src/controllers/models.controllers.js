...
/**this controller using to the handling Study materials */
...
//searching the specific note by subject and grade
exports.getNote = async (req, res) => {//exporting the getNote method
  try {
    //getting the subject and grade from the query
    const { subject,grade } = req.query;
    //find and get the notes 
    const notes = await note.findOne({ subject: subject,grade:grade});
    //checking the notes is available or not
    if (!notes) {
      // If no notes found, send custom error message
      return res.status(404).json({ msg: 'Notes not found' });
...
exports.DownloadNote = async (req, res) => {//exporting the download method
  try {
    ...
    //getting the file path
    const filePath = path.resolve('uploads','notes', Note.file);
...
//getting the specific PDF by subject and grade
exports.getOnePdf = async (req, res) => {//exporting the get PDF method
  try {
    //getting the subject and grade from the query
    const { subject,grade } = req.query;
    //finding the pdf using subject and grade
    const pdfFile = await pdf.findOne({ subject: subject,grade:grade});
    //checking the pdf is available or not
    if (!pdfFile) {
      // If no notes found, send custom error message
      return res.status(404).json({ msg: 'Notes not found' });
    ...
    console.log(error);
    res.status(500).send({ msg: 'Error occurred..! try again shortly' });//send error msg
  }
};

/*
//getting the specific PDF by subject and grade
exports.getViewPdf = async (req, res) => {//exporting the get PDF method
  try {
    //getting the subject and grade from the query
    const { subject,grade } = req.query;
    //finding the pdf using subject and grade
...
//getting the specific Research by teacher name
exports.getResearch = async (req, res) => {//exporting the get method
  try {
    //getting the teacher name as a query
    const {teacher} = req.query;
    //finding the research using teacher name
    const research = await Research.findOne({teacher:teacher});
    //checking research file available or not
    if (!research) {
      // If no notes found, send custom error message
      return res.status(404).json({ msg: 'Notes not found' });
    ...
    //sending the research as a response
    res.json(research);
  } catch (error) {//catching the errors
    console.log(error);
    res.status(500).send({ msg: 'Error occurred..! try again shortly' });//send the error msg
...
//getting the specific Record by subject and grade
exports.getRecord = async (req, res) => {//exporting the get method
  try {
    //getting the subject and grade from the query
    const { subject,grade } = req.query;
    //finding the record using subject and grade
    const records = await Record.findOne({ subject: subject,grade:grade});
    //checking the records available or not
    if (!records) {
      // If no notes found, send custom error message
      return res.status(404).json({ msg: 'Notes not found' });
